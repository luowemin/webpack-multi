(function (global, factory) {
    ...
    var vueLazyload = (function (Vue) {
        ...
        var DEFAULT_URL = 'http://7xv1io.com1.z0.glb.clouddn.com/alt.jpg?imageView2/1/w/300/h/150';
        ...
        var getRetinaUrl = function(src) {
            var scale = Math.floor(Init.scale);
            if (scale >= 2) {
                src = src.replace(/@1x|@2x/g, '@' + scale + 'x');
            }
            return src;
        };

        var setElRender = function setElRender(el, bindType, src, state, context) {
            src = getRetinaUrl(src);
        ...
        var loadImageAsync = function loadImageAsync(item, resolve, reject) {
            var image = new Image();
            image.src = getRetinaUrl(item.src);
